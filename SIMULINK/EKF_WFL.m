...
if EKF_IN.PARAM.WALL_FOLLOWER_MODE == 0 
    xA = EKF_IN.PARAM.RANGE_XA;
    yA = EKF_IN.PARAM.RANGE_YA;
else
    xA = EKF_IN.PARAM.LIDAR_2D_XA;
    yA = EKF_IN.PARAM.LIDAR_2D_YA;    
end
...
% EKF OBSVR = [RANGE_YAW_ANG ; RANGE_WALL_DIST]
switch EST_MODE
    case 0 % NOT ENABLED
        ind_x = logical([0 0 0]);
        ind_z = false(1,Nz);
...
% State equations
f1 = x1 + T*(u2 - x3);
f2 = x2 - T*(s_x1*(u1 - yA*(u2 - x3)) + xA*c_x1*(u2 - x3));
f3 = x3;
f = [ f1 f2 f3 ]';
% State-transition matrix
F1_1 = 1;
F1_3 = -T;
F2_1 = -T*(c_x1*(u1 - yA*(u2 - x3)) - xA*s_x1*(u2 - x3));
F2_2 = 1;
F2_3 = T*(xA*c_x1 - yA*s_x1);
F3_3 = 1;
% Covariance-matrix prediction
Pk = zeros(Nx,Nx);
Pk(1,1) = matQ1 + F1_1*(F1_1*P(1,1) + F1_3*P(3,1)) + F1_3*(F1_1*P(3,1) + F1_3*P(3,3));
Pk(1,2) = F2_1*(F1_1*P(1,1) + F1_3*P(3,1)) + F2_2*(F1_1*P(2,1) + F1_3*P(3,2)) + F2_3*(F1_1*P(3,1) + F1_3*P(3,3));
Pk(1,3) = F3_3*(F1_1*P(3,1) + F1_3*P(3,3));
Pk(2,1:1) = Pk(1:1,2);
Pk(2,2) = matQ2 + F2_1*(F2_1*P(1,1) + F2_2*P(2,1) + F2_3*P(3,1)) + F2_2*(F2_1*P(2,1) + F2_2*P(2,2) + F2_3*P(3,2)) + F2_3*(F2_1*P(3,1) + F2_2*P(3,2) + F2_3*P(3,3));
Pk(2,3) = F3_3*(F2_1*P(3,1) + F2_2*P(3,2) + F2_3*P(3,3));
Pk(3,1:2) = Pk(1:2,3);
Pk(3,3) = matQ3 + F3_3^2*P(3,3);
% Output equations
H = zeros(Nz,Nx);
e = zeros(Nz,1);
if ind_z(1)
