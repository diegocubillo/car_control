...
CONTROL.PARAM.MOTOR_VOLT_MAX = [BATTERY_VOLT BATTERY_VOLT]';
CONTROL.PARAM.MOTOR_VOLT_MIN = [-BATTERY_VOLT -BATTERY_VOLT]';
% NAVIGATION CONTROL LIMITS {'FORWARD VEL REF','YAW RATE REF' OR 'YAW_ANG_REF'}
CONTROL.PARAM.NAV_MV_MAX = [+0.5 +180*pi/180]';
CONTROL.PARAM.NAV_MV_MIN = [  0  -180*pi/180]';
...
CONTROL.STATE.MV_TARGET_TYPE = uint8([0 0])';
%--------------------------------------------------------------
...
CONTROL.TARGET.MISSION_WAYPOINTS = zeros(15,4);
...
CONTROL.TARGET.MISSION_WP_RADIUS = 0.05;
CONTROL.TARGET.MISSION_YA_ERROR = 5*pi/180;
CONTROL.TARGET.MISSION_COUNT = uint8(1);
%--------------------------------------------------------------
% REFERENCE SOURCE
% / 0. LOCAL / 1. PC  / 2. RC JOYSTICK / 3. RC SWITCHES
CONTROL.STATE.MV_TARGET_SOURCE = uint8([0 0])';
CONTROL.STATE.FV_TARGET_SOURCE = uint8(0);
CONTROL.STATE.YR_TARGET_SOURCE = uint8(0);
...
CONTROL.PARAM.ENC_YAW_RATE_matR = [88.9832/2 1682.83/2];
%--------------------------------------------------------------
...
CONTROL.PARAM.COMM_SAMPLING_TIME = 8*MODEL.PARAM.SAMPLING_TIME;
%--------------------------------------------------------------
% UART MODE
% / 0. NOT ENABLED / 1. ENABLED
...
CONTROL.PARAM.EMPTY_MSG = zeros(500,1,'uint8');
%--------------------------------------------------------------
% MCS RX STATUS
...
% LIDAR 2D MAP: DISTANCE (m) -> 1 deg resolution (0 - 359)
CONTROL.PARAM.LIDAR_2D_MAP = zeros(360,1);
%--------------------------------------------------------------
...
if CONTROL.PARAM.PID_DESIGN_METHOD == 0 % Frequency response
    ...
    % Damping factor
    FWD_VEL_PID.damping_factor = 0.8;
    % Design frequency (rad/s): k = w_d/w_d_P
    FWD_VEL_PID.k_wd_P = 1.4;
    % Reference weight
    FWD_VEL_PID.b = 1;
...
if CONTROL.PARAM.PID_DESIGN_METHOD == 0 % Frequency response
    % Phase margin (deg)
    YAW_RATE_PID.phase_margin_deg = 110;
    % Gain margin (dB)
    YAW_RATE_PID.gain_margin_dB = 0;
    % Design frequency (rad/s): k = w_d/w_d_P
    YAW_RATE_PID.k_wd_P = 0.9;
    % Reference weight
    YAW_RATE_PID.b = 1.5;
    % Lag phase of the integral action (deg)
...
% / 3. Control PD error   /  4. Control PD output
% / 5. Control PID error  /  6. Control PID output
YAW_ANG_PID.control_type = 1;
% Phase margin (deg)
YAW_ANG_PID.phase_margin_deg = 75;
% Gain margin (dB)
...
% / 3. Control PD error   /  4. Control PD output
% / 5. Control PID error  /  6. Control PID output
WFL_CD_PID.control_type = 2;
% Phase margin (deg)
WFL_CD_PID.phase_margin_deg = [80 60 60 60];
% Design frequency (rad/s): k = w_d/w_d_P                                          
WFL_CD_PID.k_wd_P = [0.9 0.9 0.9 0.9];                                                                 
% Reference weight                     
...
%-------------------------------------------------------------
% NAVIGATION SFC: SPECIFICATIONS [FORWARD_VEL YAW_RATE]                    
% Design method
% 1. Pole placement / 2. LQR
...
% LQR state weighting matrix
VEL_SFC.forward_vel_matQ = [1e-3 1];
VEL_SFC.yaw_rate_matQ = [0.1 1];
% LQR MV weighting matrix
VEL_SFC.forward_vel_matR = 1e-4;
...
%     K_aux = zeros(2,3);
%     Ki_aux = zeros(2,2);
%     K_aux(1,1) = CONTROL.PARAM.FORWARD_VEL_SFC.K;
%     K_aux(2,2:3) = CONTROL.PARAM.YAW_ANG_SFC.K;
%     Ki_aux(1,1) =  CONTROL.PARAM.FORWARD_VEL_SFC.Ki;
%     Ki_aux(2,2) =  CONTROL.PARAM.YAW_ANG_SFC.Ki;
% else
%     K_aux = zeros(2,5);
%     Ki_aux = zeros(2,2);
%     K_aux(1,1:3) = CONTROL.PARAM.FORWARD_VEL_SFC.K;
%     K_aux(2,4:5) = CONTROL.PARAM.YAW_ANG_SFC.K;
%     Ki_aux(1,1) =  CONTROL.PARAM.FORWARD_VEL_SFC.Ki;
%     Ki_aux(2,2) =  CONTROL.PARAM.YAW_ANG_SFC.Ki;
% end
% % Navigation model
% P_ss = minreal(LIN_MODEL(1).NAV_SS_MODEL([1 3],1:2));
% matA = P_ss.a;
% matB = P_ss.b;
% matC = P_ss.c;
% matD = P_ss.d;
...
% NAV_MPC_SS_MODEL.InputName = {'FORWARD VEL REF','YAW ANG REF'};
% NAV_MPC_SS_MODEL.InputUnit = {'m/s','rad'};
% NAV_MPC_SS_MODEL.OutputName = {'FORWARD VEL','YAW ANG'};
% NAV_MPC_SS_MODEL.OutputUnit = {'m/s','rad'};
...
% Radius for waypoint validation (m)
CONTROL.PARAM.WP_R = 0.05;
% Maximun velocity magnitude (m/s)
CONTROL.PARAM.VEL_MAG_MAX = 0.5;
...
%--------------------------------------------------------------
% LIDAR 2D
% [ANGLE(deg) DISTANCE(m)]: up to 20 measurements available each sampling time
CONTROL.INPUT.LIDAR_2D = zeros(20,2);
%--------------------------------------------------------------
% WALL RANGE SENSOR
...
CONTROL.STATE.DELAY = 0; % DELAY IN CONTROL ACTIVATION
