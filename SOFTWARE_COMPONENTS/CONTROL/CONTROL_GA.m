%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%  CONTROL TUNING BASED ON GENETIC ALGORITHM   %%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%--------------------------------------------------------------
%% FIT MODE
%--------------------------------------------------------------
...
lb = 0.1*ones(1,numVar);
ub = 2*ones(1,numVar);
FinessFunction = @(x) fitnessFcn(x,CONTROL_INI,LIN_MODEL,...
                        FIT_MODE,CONTROL_TYPE,...
                        PID_DESIGN_METHOD,SFC_DESIGN_METHOD,...
                        PARAM_LIST,PARAM_STRING,...
                        th_ini,startTime,cmd_fit,out_fit,ref_fit,factor,...
                        SFC_IN,PID_IN,MODEL_SLX);
[theta,fval] = ga(FinessFunction,numVar,[],[],[],[],lb,ub,[],options);

[V,ym,ys] = fitnessFcn(theta,CONTROL_INI,LIN_MODEL,...
                        FIT_MODE,CONTROL_TYPE,...
                        PID_DESIGN_METHOD,SFC_DESIGN_METHOD,...
                        PARAM_LIST,PARAM_STRING,...
                        th_ini,startTime,cmd_fit,out_fit,ref_fit,factor,...
...
if FIT_MODE<6
    ...
    plot(time,ym*factor(1),'-r',time,ys*factor(1),'-b',time,(ym-ys)*factor(1),'-g')
    xlabel('Time (s)')
    ylabel(plot_y_label)
    ...
    plot_y_label = OUT_STRING(OUT_STRING_POS);
    subplot(211)
    plot(time,ym(1:N_plot)*factor(1),'-r',time,ys(1:N_plot)*factor(1),'-b',time,(ym(1:N_plot)-ys(1:N_plot))*factor(1),'-g')
    xlabel('Time (s)')
    ylabel(plot_y_label{1})
    legend('Real value','Simulated value','Error')
    grid
    subplot(212)
    plot(time,ym(N_plot+1:end)*factor(2),'-r',time,ys(N_plot+1:end)*factor(2),'-b',time,(ym(N_plot+1:end)-ys(N_plot+1:end))*factor(2),'-g')
    xlabel('Time (s)')
...
%% FITNESS FUNCTION
%--------------------------------------------------------------
function [V,ym,ys] = fitnessFcn(th,CONTROL_INI,LIN_MODEL,...
                        FIT_MODE,CONTROL_TYPE,...
                        PID_DESIGN_METHOD,SFC_DESIGN_METHOD,...
                        PARAM_LIST,PARAM_STRING,...
                        th_ini,startTime,cmd_fit,out_fit,ref_fit,factor,...
